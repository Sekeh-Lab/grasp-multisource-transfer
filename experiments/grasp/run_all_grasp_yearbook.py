#!/usr/bin/env python
"""
run_all_grasp_yearbook.py - Run all GRASP experiments for Yearbook

Runs comprehensive GRASP experiments across Yearbook decades.

Usage:
    python run_all_grasp_yearbook.py --num_trials 3 --model mobilevit-xxs
    python run_all_grasp_yearbook.py --num_trials 5 --model resnet-50
"""
...
def write_summary_report(all_results: Dict, summary_file: Path, args):
    ...
    with open(summary_file, 'w') as f:
        f.write("=" * 100 + "\n")
        f.write("GRASP - YEARBOOK COMPREHENSIVE RESULTS\n")
        f.write("=" * 100 + "\n\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Run all GRASP experiments for Yearbook"
    )
    ...
    print(f"\n{'='*100}")
    print(f"GRASP COMPREHENSIVE EXPERIMENTS - YEARBOOK")
    print(f"{'='*100}")
    ...
    if all_results:
        threshold_str = str(int(args.alignment_threshold * 100))
        summary_file = Path(f"grasp_{threshold_str}_{args.model}_yearbook_summary.txt")
