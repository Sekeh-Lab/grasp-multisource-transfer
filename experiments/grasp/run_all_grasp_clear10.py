#!/usr/bin/env python
"""
run_all_grasp_clear10.py - Run all GRASP experiments for CLEAR-10

Runs comprehensive GRASP experiments across all CLEAR-10 temporal bins.

Usage:
    python run_all_grasp_clear10.py --num_trials 3 --model mobilevit-xxs
    python run_all_grasp_clear10.py --num_trials 5 --model mobilevit-small
"""
...
def write_summary_report(all_results: Dict, summary_file: Path, args):
    ...
    with open(summary_file, 'w') as f:
        f.write("=" * 100 + "\n")
        f.write("GRASP - CLEAR-10 COMPREHENSIVE RESULTS\n")
        f.write("=" * 100 + "\n\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Run all GRASP experiments for CLEAR-10"
    )
    ...
    print(f"\n{'='*100}")
    print(f"GRASP COMPREHENSIVE EXPERIMENTS - CLEAR-10")
    print(f"{'='*100}")
    ...
    if all_results:
        threshold_str = str(int(args.alignment_threshold * 100))
        summary_file = Path(f"grasp_{threshold_str}_{args.model}_clear10_summary.txt")
