#!/usr/bin/env python
"""
run_all_fusion_yearbook.py - Run all FUSION experiments for Yearbook

Runs comprehensive FUSION experiments across Yearbook temporal bins.

Usage:
    python run_all_fusion_yearbook.py --trial 3
"""
...
def write_summary_report(all_results: Dict, summary_file: Path, args):
    ...
    with open(summary_file, 'w') as f:
        f.write("=" * 100 + "\n")
        f.write("FUSION - YEARBOOK COMPREHENSIVE RESULTS\n")
        f.write("=" * 100 + "\n\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Run all FUSION experiments for Yearbook",
    ...
    print(f"\n{'='*100}")
    print(f"FUSION COMPREHENSIVE EXPERIMENTS - YEARBOOK")
    print(f"{'='*100}")
    print(f"Model:   {args.model}")
    print(f"Targets: {len(ALL_DECADES)}")
    print(f"Trials:  {args.trial} per target")
    print(f"Total:   {len(ALL_DECADES) * args.trial} experiments")
    print(f"{'='*100}\n")
    ...
    if all_results:
        summary_file = Path(f"fusion_{args.model}_yearbook_summary.txt")
