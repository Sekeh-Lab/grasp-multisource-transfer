#!/usr/bin/env python
"""
run_all_pearl_yearbook.py - Run all PEARL experiments for Yearbook

Runs comprehensive PEARL experiments across all Yearbook temporal bins.

Usage:
    python run_all_pearl_yearbook.py --num_trials 3 --model mobilevit-xxs
    python run_all_pearl_yearbook.py --num_trials 5 --model mobilevit-small
"""
...
ALL_DECADES = ["before_1950s", "1950s_1960s", "1970s_1980s", "1990s_and_later"]
DATASET = "yearbook"
...
def run_single_experiment(
    ...
    cmd = [
        ...
        "--composition_epochs", str(args.composition_epochs),
        "--adapter_dim", str(args.adapter_dim),
    ...
    experiment_name = f"{model_prefix}_{target}_pearl_trial{trial}"
...
def write_summary_report(all_results: Dict, summary_file: Path, args):
    ...
    with open(summary_file, 'w') as f:
        f.write("=" * 100 + "\n")
        f.write("PEARL - Yearbook COMPREHENSIVE RESULTS\n")
        f.write("=" * 100 + "\n\n")
        
        f.write("CONFIGURATION\n")
        f.write("-" * 100 + "\n")
        f.write(f"Dataset:           Yearbook (Binary classification)\n")
        f.write(f"Model:             {args.model}\n")
        f.write(f"Method:            PEARL\n")
        f.write(f"Trials per Target: {args.num_trials}\n")
        f.write(f"Temporal Bins:     {', '.join(ALL_DECADES)}\n")
        f.write(f"\nHyperparameters:\n")
        f.write(f"  Finetune Epochs:       {args.finetune_epochs}\n")
        f.write(f"  Composition Epochs:    {args.composition_epochs}\n")
        f.write(f"  Adapter Dimension:     {args.adapter_dim}\n")
        ...
        for target, trial_results in sorted(all_results.items()):
            sources = [y for y in ALL_DECADES if y != target]
            ...
            if stats:
                ...
                f.write(f"  Adapter Training:      {stats['flops']['adapter_training_gflops']:.2f} GFLOPs\n")
                f.write(f"  Composition Learning:  {stats['flops']['composition_learning_gflops']:.2f} GFLOPs\n\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Run all PEARL experiments for Yearbook"
    )
    ...
    parser.add_argument('--targets', nargs='+', default=ALL_DECADES,
                        help='Target decades (default: all)')
    ...
    print(f"\n{'='*100}")
    print(f"PEARL COMPREHENSIVE EXPERIMENTS - Yearbook")
    print(f"{'='*100}")
    ...
    if all_results:
        summary_file = Path(f"pearl_{args.model}_yearbook_summary.txt")
