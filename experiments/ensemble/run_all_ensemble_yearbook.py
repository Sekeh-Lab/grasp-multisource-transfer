#!/usr/bin/env python
"""
run_all_ensemble_yearbook.py - Run all ENSEMBLE experiments for Yearbook

Runs comprehensive ENSEMBLE experiments across Yearbook temporal bins.

Usage:
    python run_all_ensemble_yearbook.py --num_trials 3
"""
...
def write_summary_report(all_results: Dict, summary_file: Path, args):
    ...
    with open(summary_file, 'w') as f:
        f.write("=" * 100 + "\n")
        f.write("ENSEMBLE - YEARBOOK COMPREHENSIVE RESULTS\n")
        f.write("=" * 100 + "\n\n")
        
        f.write("CONFIGURATION\n")
        f.write("-" * 100 + "\n")
        f.write(f"Dataset:           Yearbook (Decade classification)\n")
        f.write(f"Model:             {args.model}\n")
        f.write(f"Method:            ENSEMBLE (Soft Voting)\n")
        f.write(f"Trials per Target: {args.num_trials}\n")
        f.write(f"Total Targets:     {len(ALL_DECADES)}\n")
        f.write("\n" + "=" * 100 + "\n\n")
        
        f.write("RESULTS BY TARGET\n")
        f.write("=" * 100 + "\n\n")
        
        for target in ALL_DECADES:
            if target not in all_results:
                continue
            
            trial_results = all_results[target]
            stats = calculate_statistics(trial_results)
            sources = [d for d in ALL_DECADES if d != target]
            
            f.write(f"TARGET: {target.upper().replace('_', ' ')}\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Run all ENSEMBLE experiments for Yearbook",
    ...
    print(f"\n{'='*100}")
    print(f"ENSEMBLE COMPREHENSIVE EXPERIMENTS - YEARBOOK")
    print(f"{'='*100}")
    print(f"Model:   {args.model}")
    print(f"Targets: {len(ALL_DECADES)}")
    print(f"Trials:  {args.num_trials} per target")
    print(f"Total:   {len(ALL_DECADES) * args.num_trials} experiments")
    print(f"{'='*100}\n")
    ...
    if all_results:
        summary_file = Path(f"ensemble_{args.model}_yearbook_summary.txt")
