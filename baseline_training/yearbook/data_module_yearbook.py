"""
data_module_yearbook.py - PyTorch Lightning Data Module for Yearbook Dataset

This module handles data loading, preprocessing, and augmentation for the Yearbook
dataset with support for both HuggingFace transformers and torchvision CNN models.
"""
...
class YearbookDataModule(pl.LightningDataModule):
    """
    PyTorch Lightning Data Module for Yearbook decade classification.
    ...
    def __init__(
        ...
        if subset_name not in self.SUBSET_DECADES:
            raise ValueError(
                f"Invalid subset_name '{subset_name}'. "
                f"Must be one of: {list(self.SUBSET_DECADES.keys())}"
            )
        
        self.class_names = self.SUBSET_DECADES[subset_name]
        
        self.subset_dir = self.data_root / subset_name
        self.train_dir = self.subset_dir / "train"
        self.val_dir = self.subset_dir / "val"
        self.test_dir = self.subset_dir / "test"
    ...
    def _compute_class_weights(self) -> torch.Tensor:
        ...
        for i, (name, weight) in enumerate(zip(self.class_names, class_weights)):
    ...
    def get_class_names(self) -> List[str]:
        """Get the class names for this subset."""
