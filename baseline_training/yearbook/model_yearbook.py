"""
model_yearbook.py - PyTorch Lightning Module for Yearbook Decade Classification

This module handles the model architecture, training, and evaluation for Yearbook dataset.
Supports both transformer models (HuggingFace) and CNN models (torchvision).

Each subset has 2 decade classes:
...
MODEL_REGISTRY = {
    ...
    'mobilevit-xs': {
        ...
        'short_name': 'mvit-xs',
        'display_name': 'MobileViT-X-Small',
        'params': 2.3,
        'recommended_lr': 3e-4,
        'recommended_batch_size': 32,
        'description': 'Lightweight transformer'
    },
    'mobilevit-small': {
        'type': 'huggingface',
...
def get_model_info(model_key: str) -> Dict[str, Any]:
    ...
    warnings.warn(
        ...
        f"To use a custom model, add it to MODEL_REGISTRY in model_yearbook.py\n"
...
class YearbookDecadeClassifier(pl.LightningModule):
    """
    PyTorch Lightning Module for binary decade classification on Yearbook dataset.
    ...
    def __init__(
        ...
        if class_names is None:
            self.class_names = ['Decade 1', 'Decade 2']
        else:
        ...
        print(f"Classification head initialized with {num_classes} classes: {self.class_names}")
    ...
    def on_validation_epoch_end(self):
        ...
        self.log('val_tn', float(tn), logger=True)
        self.log('val_fp', float(fp), logger=True)
        self.log('val_fn', float(fn), logger=True)
        self.log('val_tp', float(tp), logger=True)
        ...
        if self.trainer.is_global_zero:
            ...
            print(f"\n  Confusion Matrix:")
            print(f"                Predicted {self.class_names[0]:<8s}  Predicted {self.class_names[1]:<8s}")
            print(f"    Actual {self.class_names[0]:<8s}: {tn:7.0f}      {fp:7.0f}")
            print(f"    Actual {self.class_names[1]:<8s}: {fn:7.0f}      {tp:7.0f}")
            print(f"{'='*80}\n")
    ...
    def on_test_epoch_end(self):
        ...
        self.log('test_tn', float(tn), logger=True)
        self.log('test_fp', float(fp), logger=True)
        self.log('test_fn', float(fn), logger=True)
        self.log('test_tp', float(tp), logger=True)
...
YearbookGenderClassifier = YearbookDecadeClassifier