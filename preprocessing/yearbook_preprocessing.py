...
Binary classification (Male/Female) across 4 decade-based periods.
"""
import os
import shutil
import re
...
def collect_files_by_period():
    """Scan directories and collect files by period and gender."""
    files_by_period = defaultdict(lambda: {'M': [], 'F': []})
    ...
    for gender in ['M', 'F']:
        ...
        for idx, filename in enumerate(image_files):
            ...
            files_by_period[period_name][gender].append((full_path, filename, year))
...
def stratified_split(files_dict, train_ratio=0.8, val_ratio=0.1, test_ratio=0.1):
    """Split files by gender maintaining train/val/test ratios."""
    splits = {'train': {'M': [], 'F': []}, 
              'val': {'M': [], 'F': []}, 
              'test': {'M': [], 'F': []}}
    
    for gender in ['M', 'F']:
        files = files_dict[gender].copy()
        ...
        splits['train'][gender] = files[:train_end]
        splits['val'][gender] = files[train_end:val_end]
        splits['test'][gender] = files[val_end:]
...
def copy_files_and_collect_stats(splits, period_name, output_base):
    ...
    for split_name in ['train', 'val', 'test']:
        for gender in ['M', 'F']:
            dest_dir = os.path.join(output_base, period_name, split_name, gender)
            os.makedirs(dest_dir, exist_ok=True)
            
            files = splits[split_name][gender]
            ...
            for idx, (src_path, filename, year) in enumerate(files):
                ...
                try:
                    ...
                    stats[split_name]['by_gender'][gender] += 1
                    stats[split_name]['total'] += 1
                except Exception as e:
                    print(f"[WARNING] Copy failed: {filename}")
                
                if (idx + 1) % 5000 == 0:
                    print(f"    {split_name}/{gender}: {idx + 1}/{len(files)}")
...
def organize_dataset():
    ...
    for period_name in period_order:
        ...
        total = sum(len(files[gender]) for gender in ['M', 'F'])
        ...
        print(f"Male: {len(files['M'])}")
        print(f"Female: {len(files['F'])}")
        ...
        for split in ['train', 'val', 'test']:
            ...
            print(f"M: {stats[split]['by_gender']['M']}, F: {stats[split]['by_gender']['F']}")
    
    return all_stats, period_order


def print_summary(all_stats, period_order):
    """Print overall summary statistics."""
    print("Summary Statistics")
    
    grand_total = 0
    total_by_split = {'train': 0, 'val': 0, 'test': 0}
    total_by_gender = {'M': 0, 'F': 0}
    ...
    for period in period_order:
        ...
        for split in ['train', 'val', 'test']:
            if split in all_stats[period]:
                ...
                for gender in ['M', 'F']:
                    total_by_gender[gender] += all_stats[period][split]['by_gender'][gender]
...
def main():
    ...
    if result:
        all_stats, period_order = result
        print_summary(all_stats, period_order)
